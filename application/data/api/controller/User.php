...
class User extends Online
{
    ...
    public function get_profit(){
        if(request()->isPost()){
            ...
            $this->returnMsg['data']['total_account']=$total_accountmonth_account;
...
//        $this->returnMsg['data']['sye_rate']=$this->sye_rate;
    ...
    public function advance(){
        ...
        if(request()->isPost()){
            ...
            if(empty($advance_amount) || $advance_amount<=2){
                $this->returnMsg['message']='分润提现金额不能低于两元';
                return $this->returnMsg;
            }
			$service=2;
            ...
            $user_res=Db::table('user')
                ->where('user_id',$this->online['user_id'])
                ->update(['balance'=>$merchant_data['balance']-$advance_amount - $service]);
            if(!$user_res){
            ...
            //$this->returnMsg['res']=$result;
            $order_res=$result['data'];

            if(empty($order_res)){
                $this->returnMsg['message'] = '订单生成失败1';
                Db::rollback();
                return $this->returnMsg;
            }
            if($order_res['resp_code'] =='PAY_FAILURE' ){
                $this->returnMsg['message'] = '订单生成失败2';
                Db::rollback();
                return $this->returnMsg;
            }

            //$this->returnMsg['datt']=$order_res['resp_code'];

            $pay_data=[];
            $pay_data['pay_order_id']=$result['client_trans_id'];
            $pay_data['pay_money']=$advance_amount;
            $pay_data['pay_to_card']=$this->online['debit_card'];
            $pay_data['pay_time']=$result['trans_timestamp'];
            $pay_data['pay_status']=$result['data']['resp_code'] ;
