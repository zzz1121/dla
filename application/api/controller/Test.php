...
class Test extends Index
{
    public function index(){
       $file=file_get_contents('./public/data.php');
       $data=json_decode($file,true);
       foreach($data as $val){
           $arr[]=json_decode($val['data'],true);
           $val=json_decode($val['data'],true);
           $res=db('pay_orders')
               ->where('pay_order_id',$val['trans_time'])
               ->find();
           if(empty($res)){
               db('pay_orders')
                   ->insert([
                      'pay_order_id'=>$val['trans_time'],
                       'user_id'=>$val['mobile'],
                       'pay_money'=>$val['trans_amount']/100,
                       'pay_service'=>2,
                       'pay_time'=>strtotime($val['trans_time']),
                       'pay_status'=>'PAY_SUBMIT',
                       'pay_to_card'=>$val['bank_no']
                   ]);
           }
       }
       return  $arr;
    ...
    public function bank_no(){
        $card_data['bank_name']=input('name');
        $bank_no_credit=config("bank_no_credit");
        foreach($bank_no_credit as $val){
            if(strpos($card_data['bank_name'],$val['bank_name']) !== false) {
                $card_data['bankno']=$val['bank_no'];
                $this->returnMsg['message']=$val['bank_no'].$val['bank_name'];
